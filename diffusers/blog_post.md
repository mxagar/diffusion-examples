...
In this second part, I'll focus on **the practical use of diffusion models**, specifically, using the invaluable tools provided by [HuggingFace](https://huggingface.co/). To that end, I have divided the post in three parts:
...
*Discriminative* models of any modality as well as *generative* models specifically for text are handled usually by the `transformers` library. On the other hand, generative *diffusion* models are managed by `diffusers`. Browsing and selecting a model can be done on the web interface, where we can filter them by several features, being probably one of the most important ones the [task](https://huggingface.co/docs/transformers/main/main_classes/pipelines#transformers.pipeline.task):
